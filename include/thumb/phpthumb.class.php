	...
	function GenerateThumbnail() {
		...
		switch ($this->far) {
			...
			default:
				...
				$match=array(0,'0','0');
				$m=preg_match("/(x\d+)(y\d+)/", $this->far,$match);
				$match[1]=str_replace('x','',$match[1]);
				$match[2]=str_replace('y','',$match[2]);
				$destination_offset_x =$m ? intval($match[1]) : round(($this->thumbnail_width  - $this->thumbnail_image_width)  / 2);
				$destination_offset_y =$m ? intval($match[2]) : round(($this->thumbnail_height - $this->thumbnail_image_height) / 2);
				$this->DebugMessage('match '.print_r($match,true).' m='.$m."dx $destination_offset_x dy ".round(($this->thumbnail_height - $this->thumbnail_image_height) / 2));
		}
